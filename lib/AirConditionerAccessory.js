...
class AirConditionerAccessory extends BaseAccessory {
    ...
    setRotationSpeed(value, callback) {
        const {Characteristic} = this.hap;
        const cdpActive = this.dpActive;
        const cdpRotationSpeed = this.dpRotationSpeed;
        
        if (value === 0) {
            this.setActive(Characteristic.Active.INACTIVE, callback);
        } else {
            this._hkRotationSpeed = value;
            this.setMultiState({cdpActive: true, cdpRotationSpeed: this.convertRotationSpeedFromHomeKitToTuya(value)}, callback);
        }
    }
