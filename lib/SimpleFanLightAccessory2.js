...
class SimpleFanLightAccessory2 extends BaseAccessory {
	...
	if (this.useLight) {
			...
			minValue: 0,
			maxValue: 600})
		    .updateValue(dps[this.dpColorTemperature])
	    ...
	    if (changes.hasOwnProperty(this.dpRotationDirection) && characteristicRotationDirection.value !== changes[this.dpRotationDirection])
            ...
            if (changes.hasOwnProperty(this.dpColorTemperature)) {
                if (this.characteristicColorTemperature.value !== changes[this.dpColorTemperature])
                    characteristicColorTemperature.updateValue(changes[this.dpColorTemperature]);
            } else if (changes[this.dpBrightness]) {
                characteristicColorTemperature.updateValue(state[this.dpColorTemperature]);
            }
...
// Lightbulb Brightness
    ...
    getColorTemperature(callback) {
        callback(null, this.device.state[this.dpColorTemperature]);
    }
	...
	//this.setState(this.dpColorTemperature, this.convertColorTemperatureFromHomeKitToTuya(value), callback);
        this.setState(this.dpColorTemperature, value, callback);
    }
