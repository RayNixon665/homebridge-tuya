...
class SimpleFanLightAccessory2 extends BaseAccessory {
	...
	this.dpRotationDirection = this._getCustomDP(this.device.context.RotationDirection) || '102';
	...
	console.log("MainUpdate Call Tuya Value - " + dps[this.dpColorTemperature]);
        console.log("MainUpdate Call Converted to Homekit Value - " + this.convertColorTemperatureFromTuyaToHomeKit(dps[this.dpColorTemperature]));
		//console.log("Update Value - " + this.convertColorTemperatureFromTuyaToHomeKit(dps[this.dpColorTemperature]));
...
// Lightbulb Brightness
    ...
    getColorTemperature(callback) {
	 ...
	 console.log("dpColour - " + this.device.state[this.dpColorTemperature]);
         console.log("GetColourConverted - " + this.convertColorTemperatureFromTuyaToHomeKit(this.device.state[this.dpColorTemperature]));
    }
	...
	scaleDown (input) {
		...
		let outMin = 0;
		let outMax = 100;
	    return Math.round((input - inMin) * (outMax - outMin) / (inMax - inMin) + outMin);
	}
