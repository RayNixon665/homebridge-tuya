...
class SimpleFanLightAccessory2 extends BaseAccessory {
	...
	//const characteristicRotationDirection = serviceFan.getCharacteristic(Characteristic.RotationDirection)
	  // .updateValue(this._getRotation(this.ConvertRotation(dps[this.dpRotationDirection])))
	//		.updateValue(1)
	...
	console.log("MainUpdate Call Tuya Value (post scale Up) - " + this.scaleChar(dps[this.dpColorTemperature]));
	   ...
	   // if (changes.hasOwnProperty(this.dpRotationDirection) && characteristicRotationDirection.value !== changes[this.ConvertRotation(dps[this.dpRotationDirection])])
             //   characteristicRotationDirection.updateValue(changes[this.ConvertRotation(dps[this.dpRotationDirection])]);
...
// Fan Speed
  ...
  //  getRotation(callback) {
    //   this.getState(this.ConvertRotation(this.dps[this.dpRotationDirection]), (err, dp) => {
	//       console.log("Get Rotation - " + this.ConvertRotation(this.dps[this.dpRotationDirection]));
          //  if (err) return callback(err);
